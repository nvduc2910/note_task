...
@IBDesignable class PlaceholderTextView: UITextView {
    ...
    var fontSize: CGFloat? {didSet {setNeedsDisplay() }}
    ...
    func textChangedForPlaceholderTextView(_ notification: Notification) {
        if let size = fontSize {
            self.font = UIFont(name: "Lato-Medium", size: size)
        }else{
            
            self.font = self.text.characters.count == 0 ? UIFont(name: "Lato-Medium", size: 14)  : UIFont(name: "Lato-Medium", size: 14)
        }
        setNeedsDisplay()
        setNeedsLayout()
    }
